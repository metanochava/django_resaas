...
def resaas_action(
    *,
    ...
    """Declara uma action DRF e adiciona metadados RESAAS ao método."""
    """
    ...
    methods = [str(method).lower() for method in (methods or ["post"])]
    ...
    def decorator(func):
        ...
        func._resaas_action = {
            "action": action_name,
            "label": label or action_name.replace("_", " ").title(),
            "icon": icon,
            "tooltip": tooltip,
            "position": position,
            "order": order,
