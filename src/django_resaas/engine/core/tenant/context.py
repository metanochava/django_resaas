...
class ResaasContextService:
    ...
    def validate_entity(
        ...
        allowed = EntityUser.objects.filter(
            entity=entity,
            user=user,
    ...
    def validate_branch(
        ...
        try:
            ...
            raise PermissionDenied(
                "Invalid branch for this entity."
            )
        ...
        allowed = BranchUser.objects.filter(
            branch=branch,
            user=user,
            deleted_at__isnull=True,
        ).exists()
    ...
    def validate_group(
        ...
        if not branch:
            ...
            raise ValidationError({
                "group_id": "A group requires a branch."
            })
        ...
        allowed = BranchUserGroup.objects.filter(
            branch=branch,
            user=user,
            group_id=group_id,
            deleted_at__isnull=True,
    ...
    def issue(
        ...
        entity = cls.validate_entity(
            user,
            entity_id,
        )


        branch = cls.validate_branch(
            user,
            entity,
            branch_id,
        )
    ...
    def decode(
        ...
        if payload.get("version") != cls.VERSION:
            ...
            raise PermissionDenied(
                "Unsupported RESAAS context version."
            )
    ...
    def validate_for_user(
        ...
        if str(
            ...
            payload.get("entity_type_id")
        ):

            raise PermissionDenied(
                "Invalid entity type context."
            )
